...
class FeedImporter:
    ...
    def run(self):
        ...
        for offer in root.findall("y:offer", ns):
            ...
            """
            ProjectParams.objects.update_or_create(
                project=project,
                defaults={
                    "city": city,
                    "district": district,
    ...
    def update_instance(self, instance, **values) -> bool:
        """
        Обновляет только реально изменившиеся поля.

        Returns:
            True  - если объект был изменен и сохранен.
    ...
    def touch_instance(self, instance, created, stats, created_key, updated_key, **values):
        """
        created=True  -> увеличивает *_created
        created=False -> сохраняет только реально изменившиеся поля
        """

        changed = self.update_instance(instance, **values)
    ...
    def update_or_create_changed(self, model, lookup, defaults):
